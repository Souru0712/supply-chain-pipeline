...
def _synthetic_pair() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    fc = pd.DataFrame({
        ...
        "p10": p50 - spread, "p50": p50, "p90": p50 + spread,
    })
    return hist, fc
...
TAB_SELECTED = {"padding": "12px 20px", "fontWeight": "700", "borderTop": "3px solid #2980b9"}
...
def compute_risk(hist: pd.DataFrame, fc: pd.DataFrame) -> dict:
    ...
    color = "#27ae60" if composite < 33 else ("#e67e22" if composite < 66 else "#e74c3c")
    return {"composite": composite, "level": level, "color": color,
            "volatility": vol_score, "uncertainty": unc_score, "momentum": mom_score}


def build_forecast_fig(hist: pd.DataFrame, fc: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=hist["week_start"], y=hist["price_avg_weekly"],
                             name="Historical", line={"color": "#2980b9", "width": 2.5}, mode="lines"))
    ...
    fig.add_trace(go.Scatter(x=x_band, y=y_band, fill="toself",
                             fillcolor="rgba(231,76,60,0.15)", line={"color": "rgba(0,0,0,0)"},
                             name="80% CI", hoverinfo="skip"))
    fig.add_trace(go.Scatter(x=fc["target_week_start"], y=fc["p50"], name="Forecast p50",
                             line={"color": "#e74c3c", "width": 2.5, "dash": "dash"},
                             mode="lines+markers", marker={"size": 5}))
    for col, label in [("p10", "p10"), ("p90", "p90")]:
        fig.add_trace(go.Scatter(x=fc["target_week_start"], y=fc[col], name=label,
                                 line={"color": "#e74c3c", "width": 1, "dash": "dot"},
                                 mode="lines", opacity=0.5))
    fd = str(pd.Timestamp(fc["forecast_date"].iloc[0]).date())
    fig.add_shape(type="line", x0=fd, x1=fd, y0=0, y1=1, yref="paper",
                  line={"dash": "dot", "color": "gray", "width": 1.5})
    fig.update_layout(title="13-Week Corn Price Forecast (LightGBM)", xaxis_title="Date",
                      yaxis_title="Price ($/bushel)", template="plotly_white",
                      hovermode="x unified", height=480, margin={"t": 80},
...
def build_gauge_fig(value: float, title: str, level: str, color: str) -> go.Figure:
    fig = go.Figure(go.Indicator(
        ...
        title={"text": f"{title}<br><span style='font-size:18px;color:{color}'>{level}</span>"},
...
def _bar_row(label: str, value: int) -> html.Div:
    return html.Div([
        html.Div(label, style={"width": "180px", "fontWeight": "600", "fontSize": "14px"}),
        html.Div(style={"flex": "1", "background": "#ecf0f1", "borderRadius": "4px",
                        "height": "22px", "margin": "0 12px", "overflow": "hidden"},
                 children=[html.Div(style={"width": f"{value}%", "background": color_for(value),
                                           "height": "100%", "borderRadius": "4px"})]),
        html.Div(f"{value}/100", style={"width": "60px", "textAlign": "right", "fontSize": "14px"}),
...
def build_forecast_table(fc: pd.DataFrame) -> html.Table:
    header = html.Tr([
        ...
        for h in ["Horizon", "Week of", "p10 ($/bu)", "p50 ($/bu)", "p90 ($/bu)", "Spread"]
    ], style={"background": "#2c3e50", "color": "white"})
    rows = []
    for _, row in fc.iterrows():
        spread = row["p90"] - row["p10"]
        rows.append(html.Tr([
            html.Td(f"Week +{int(row['horizon_weeks'])}", style={"padding": "8px 14px"}),
            html.Td(pd.Timestamp(row["target_week_start"]).strftime("%b %d, %Y"),
                    style={"padding": "8px 14px"}),
            html.Td(f"${row['p10']:.3f}", style={"padding": "8px 14px", "color": "#2980b9"}),
            html.Td(f"${row['p50']:.3f}", style={"padding": "8px 14px", "fontWeight": "700"}),
            html.Td(f"${row['p90']:.3f}", style={"padding": "8px 14px", "color": "#e74c3c"}),
            html.Td(f"±${spread/2:.3f}", style={"padding": "8px 14px", "color": "#7f8c8d"}),
...
def build_shap_tab(hist: pd.DataFrame, fc: pd.DataFrame) -> html.Div:
    ...
    if shap_df is not None and "mean_importance" in shap_df.columns:
        ...
        fig.update_layout(title="Top Feature Importances — Mean |SHAP| (median model, all horizons)",
                          xaxis_title="Mean |SHAP value|", template="plotly_white",
                          height=420, margin={"t": 60, "l": 200})
    ...
    if meta2:
        ...
        if dir_rows:
            ...
            fig2 = go.Figure(go.Bar(
                x=da_df["horizon"], y=(da_df["directional_accuracy"] * 100).round(1),
                marker_color="#27ae60", text=(da_df["directional_accuracy"] * 100).round(1),
                texttemplate="%{text:.1f}%", textposition="outside",
            ))
            fig2.add_hline(y=50, line_dash="dash", line_color="gray",
                           annotation_text="50% (random guess)")
            fig2.update_layout(title="Directional Accuracy by Horizon (% correct up/down calls)",
                               xaxis_title="Horizon (weeks)", yaxis_title="Directional Accuracy (%)",
                               yaxis_range=[0, 105], template="plotly_white",
                               height=360, margin={"t": 60})
...
def build_margin_tab() -> html.Div:
    ...
    if margin_df is not None:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=recent["week_start"], y=recent["price_avg_weekly"],
                                 name="Commodity Price", stackgroup="cost",
                                 line={"color": "#2980b9"}, mode="lines"))
        fig.add_trace(go.Scatter(x=recent["week_start"], y=recent["transport_cost"],
                                 name="Transport", stackgroup="cost", mode="lines",
                                 line={"color": "#e67e22"}))
        fig.add_trace(go.Scatter(x=recent["week_start"], y=recent["energy_cost"],
                                 name="Energy", stackgroup="cost", mode="lines",
                                 line={"color": "#8e44ad"}))
        fig.add_trace(go.Scatter(x=recent["week_start"], y=recent["carry_cost"],
                                 name="Carry Cost", stackgroup="cost", mode="lines",
                                 line={"color": "#e74c3c"}))
        fig.add_trace(go.Scatter(x=recent["week_start"], y=recent["implied_sell_price"],
                                 name="Implied Sell Price", mode="lines",
                                 line={"color": "#27ae60", "width": 2.5, "dash": "dash"}))
        fig.update_layout(title="Total Landed Cost Stack — Last 52 Weeks",
                          xaxis_title="Date", yaxis_title="USD / bushel",
                          template="plotly_white", height=460, margin={"t": 60},
                          legend={"orientation": "h", "yanchor": "bottom", "y": 1.02,
                                  "xanchor": "right", "x": 1})
        children.append(html.Div(dcc.Graph(figure=fig), style=CARD))
        ...
        for label, col, color in [
            ...
            cards.append(html.Div([
                ...
                html.Div(f"${val:.4f}", style={"fontSize": "22px", "fontWeight": "700", "color": color}),
                html.Div(f"{pct:.1f}% of TLC", style={"fontSize": "12px", "color": "#aaa"}),
            ], style={"background": "white", "borderRadius": "8px", "padding": "16px 20px",
                      "boxShadow": "0 2px 8px rgba(0,0,0,0.08)", "flex": "1",
                      "borderTop": f"4px solid {color}"}))
        children.append(html.Div(cards, style={"display": "flex", "gap": "16px",
                                               "marginBottom": "20px"}))
    ...
    if scenarios:
        ...
        fig2 = go.Figure(go.Bar(x=bar_labels, y=bar_vals, marker_color=bar_colors,
                                text=[f"${v:.4f}" for v in bar_vals],
                                textposition="outside"))
        fig2.add_hline(y=base, line_dash="dash", line_color="gray",
                       annotation_text=f"Base ${base:.4f}/bu")
        fig2.update_layout(title="Scenario Analysis — Total Landed Cost under +20% Cost Shocks",
                           yaxis_title="Total Landed Cost ($/bu)", template="plotly_white",
                           height=380, margin={"t": 60})
        children.append(html.Div(dcc.Graph(figure=fig2), style=CARD))

    if not children:
        children = [html.Div("Run `python scripts/9.margin_model.py` to generate margin data.",
...
def build_disruption_tab() -> html.Div:
    ...
    alert = snapshot["alert"] if snapshot else {"level": "UNKNOWN", "color": "#888",
                                                "action": "Run disruption script"}
    ...
    children.append(html.Div([
        ...
        html.Div([
            html.H3("Alert", style={"marginTop": "0", "color": alert["color"]}),
            html.P(alert["action"], style={"fontSize": "16px", "fontWeight": "600"}),
            html.Hr(),
            html.H4("Component Scores"),
            *([_bar_row(k.replace("_", " ").title(), int(v))
               for k, v in snapshot["components"].items()] if snapshot else []),
        ], style={**CARD, "flex": "1"}),
    ...
    if hist_df is not None and "disruption_score" in hist_df.columns:
        ...
        for comp, color, dash in [
            ...
            if comp in hist_df.columns:
                fig.add_trace(go.Scatter(x=hist_df["week_start"], y=hist_df[comp],
                                         name=comp.replace("score_", "").replace("_", " ").title(),
                                         line={"dash": dash, "width": 1}, opacity=0.7))
        fig.add_trace(go.Scatter(x=hist_df["week_start"], y=hist_df["disruption_score"],
                                 name="Composite", line={"color": "black", "width": 2.5}))
        for thresh, label, color in [(60, "Alert", "#fbc02d"), (75, "Review", "#f57c00"),
                                      (90, "Emergency", "#d32f2f")]:
            fig.add_hline(y=thresh, line_dash="dash", line_color=color,
                          annotation_text=label, annotation_position="right")
        fig.update_layout(title="Disruption Score History — All Components",
                          xaxis_title="Date", yaxis_title="Score (0–100)",
                          yaxis_range=[0, 105], template="plotly_white",
                          height=440, margin={"t": 60},
                          legend={"orientation": "h", "yanchor": "bottom", "y": 1.02,
                                  "xanchor": "right", "x": 1})
        children.append(html.Div(dcc.Graph(figure=fig), style=CARD))
...
def build_procurement_tab() -> html.Div:
    ...
    if sched is not None:
        ...
        fig.add_trace(go.Scatter(
            x=sched["week_index"], y=sched["p75_price"], name="p75 band",
            line={"color": "rgba(231,76,60,0.3)"}, fill=None, mode="lines"))
        fig.add_trace(go.Scatter(
            x=sched["week_index"], y=sched["p25_price"], name="p25 band",
            fill="tonexty", fillcolor="rgba(231,76,60,0.1)",
            line={"color": "rgba(231,76,60,0.3)"}, mode="lines"))
        fig.add_trace(go.Scatter(
            x=sched["week_index"], y=sched["expected_price"], name="Expected Price",
            line={"color": "#e74c3c", "width": 2.5}, mode="lines"))

        # Opportunistic weeks
        opp = sched[sched["opportunistic_flag"] == True] if "opportunistic_flag" in sched.columns else pd.DataFrame()  # noqa
        if not opp.empty:
            fig.add_trace(go.Scatter(
                x=opp["week_index"], y=opp["expected_price"], name="Opportunistic Buy",
                mode="markers", marker={"color": "#27ae60", "size": 12, "symbol": "star"}))

        fig.update_layout(title="52-Week Forward Price Curve with Seasonal Bands",
                          xaxis_title="Week Forward", yaxis_title="Expected Price ($/bu)",
                          template="plotly_white", height=380, margin={"t": 60},
                          legend={"orientation": "h", "yanchor": "bottom", "y": 1.02,
                                  "xanchor": "right", "x": 1})
        children.append(html.Div(dcc.Graph(figure=fig), style=CARD))
        ...
        fig2 = go.Figure()
        bar_colors = ["#27ae60" if row.get("opportunistic_flag", False) else "#2980b9"
                      for _, row in sched.iterrows()]
        fig2.add_trace(go.Bar(
            x=sched["week_index"],
            y=sched["recommended_purchase"] / annual_vol * 100,
            name="Purchase %", marker_color=bar_colors,
        ))
        fig2.update_layout(title="Recommended Weekly Purchase Allocation (% of Annual Volume)<br>"
                                  "<sup>Green = opportunistic buy window</sup>",
                           xaxis_title="Week Forward", yaxis_title="% of Annual Volume",
                           template="plotly_white", height=320, margin={"t": 80})
        children.append(html.Div(dcc.Graph(figure=fig2), style=CARD))

    if not children:
        children = [html.Div("Run `python scripts/11.procurement_optimizer.py` to generate schedule.",
                             style={**CARD, "color": "#888", "textAlign": "center"})]
    return html.Div(children, style=PAD)
...
def build_risk_tab() -> html.Div:
    ...
    cls = snapshot["classification"] if snapshot else {"level": "UNKNOWN", "color": "#888",
                                                       "action": "Run risk script"}
    ...
    if snapshot and "radar" in snapshot:
        ...
        radar_fig.add_trace(go.Scatterpolar(r=vals_closed, theta=cats_closed,
                                             fill="toself", name="Risk Score",
                                             line={"color": cls["color"]}))
        radar_fig.update_layout(polar={"radialaxis": {"range": [0, 100]}},
                                 title="Risk Dimensions", template="plotly_white",
                                 height=320, margin={"t": 60})
    ...
    if snapshot and "dimensions" in snapshot:
        ...
        for dim, val in snapshot["dimensions"].items():
            ...
            dim_info.append(_bar_row(
                f"{dim.replace('_', ' ').title()} ({w:.0%})", int(val)
            ))
    ...
    if dim_info:
        children.append(html.Div([
            ...
            html.P(cls["action"], style={"color": cls["color"], "fontWeight": "600"}),
            *dim_info,
        ], style=CARD))
    ...
    if hist_df is not None and "composite_risk" in hist_df.columns:
        ...
        colors_map = {"supply_risk": "#e67e22", "cost_risk": "#e74c3c",
                      "logistics_risk": "#8e44ad", "demand_risk": "#2980b9"}
        for col in dim_cols:
            if col in hist_df.columns:
                fig.add_trace(go.Scatter(
                    x=hist_df["week_start"], y=hist_df[col],
                    name=col.replace("_", " ").title(),
                    line={"color": colors_map[col], "width": 1.5, "dash": "dot"}, opacity=0.7))
        fig.add_trace(go.Scatter(
            x=hist_df["week_start"], y=hist_df["composite_risk"],
            name="Composite Risk", line={"color": "black", "width": 2.5}))
        for thresh, label, color in [(40, "Moderate", "#fbc02d"), (60, "High", "#f57c00"),
                                      (80, "Critical", "#d32f2f")]:
            fig.add_hline(y=thresh, line_dash="dash", line_color=color,
                          annotation_text=label, annotation_position="right")
        fig.update_layout(title="Risk Score History — All Dimensions",
                          xaxis_title="Date", yaxis_title="Score (0–100)",
                          yaxis_range=[0, 105], template="plotly_white",
                          height=440, margin={"t": 60},
                          legend={"orientation": "h", "yanchor": "bottom", "y": 1.02,
...
meta = _load_metadata()
train_end = (meta.get("train_end_date", "")[:10] if meta
             else str(hist_df["week_start"].max().date()))
is_demo = " ⚠️ demo data — run pipeline for live results" if _load_historical() is None else ""

risk = compute_risk(hist_df, forecast_df)
forecast_fig = build_forecast_fig(hist_df, forecast_df)
gauge_fig = build_gauge_fig(risk["composite"], "Composite Risk Score",
                             risk["level"], risk["color"])
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(style={
            ...
            html.Div([
                html.H1("🌽 Supply Chain Intelligence Dashboard",
                        style={"margin": "0", "fontSize": "26px", "fontWeight": "700"}),
                html.P(f"CORN · LightGBM · Trained through {train_end}{is_demo}",
                       style={"margin": "6px 0 0 0", "opacity": "0.85", "fontSize": "14px"}),
            ]),
            html.Div([
                html.Div(f"{risk['composite']}",
                         style={"fontSize": "42px", "fontWeight": "900",
                                "color": risk["color"], "lineHeight": "1"}),
                html.Div("Risk Score", style={"fontSize": "11px", "opacity": "0.8",
                                              "marginTop": "2px"}),
            ], style={"textAlign": "center"}),
        ]),

        # Tabs
        dcc.Tabs(style={"margin": "20px 32px 0"},
                 colors={"border": "#ddd", "primary": "#2980b9", "background": "#f4f6f9"},
                 children=[

            dcc.Tab(label="📈  13-Week Forecast", style=TAB_STYLE,
                    selected_style={**TAB_SELECTED, "borderTop": "3px solid #2980b9"},
                    children=[html.Div(style=PAD, children=[
                        html.Div(dcc.Graph(figure=forecast_fig), style=CARD),
                        html.H3("Weekly Forecast Detail", style={"margin": "0 0 12px"}),
                        build_forecast_table(forecast_df),
                    ])]),

            dcc.Tab(label="⚠️  Risk Score", style=TAB_STYLE,
                    selected_style={**TAB_SELECTED, "borderTop": "3px solid #e74c3c"},
                    children=[html.Div(style=PAD, children=[
                        html.Div([
                            html.Div(dcc.Graph(figure=gauge_fig), style={**CARD, "flex": "1"}),
                            html.Div([
                                html.H3("Risk Components", style={"marginTop": "0"}),
                                _bar_row("Price Volatility", risk["volatility"]),
                                _bar_row("Forecast Uncertainty", risk["uncertainty"]),
                                _bar_row("Price Momentum", risk["momentum"]),
                            ], style={**CARD, "flex": "1"}),
                        ], style={"display": "flex", "gap": "20px"}),
                        html.Div([
                            html.H4("Methodology", style={"marginTop": "0"}),
                            html.P("Composite = 40% × Volatility + 40% × Uncertainty + 20% × Momentum. "
